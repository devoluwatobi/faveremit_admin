...
import 'failed-withdrawal-reason.dart';

class ReviewWithdrawalOptionsList extends StatelessWidget {
  final int trxID;
  final bool canApprove;
  const ReviewWithdrawalOptionsList({
    Key? key,
    required this.trxID,
                ...
                children: [
                  ...
                  ReviewTrxOption(
                    ...
                    onPressed: () {
                      if (canApprove) {
                        _acceptWithdrawal(
                            context: context, trxID: trxID, statusID: 4);
                      } else {
                        showInfoModal(
                            context: context,
                            title: "Oops!",
                  ...
                  ReviewTrxOption(
                    ...
                    onPressed: () async {
                      await showCupertinoModalBottomSheet(
                        ...
                        builder: (context) {
                          return FailedWithdrawalReason(trxID: trxID);
                        },
                      );
                    },
                  ),
...
void _acceptWithdrawal(
  ...
  if (_proceed != null && _proceed) {
    showLoadingModal(context: context, title: "Updating Withdrawal Status");
    ProcessError _error =
        await adminWorker.acceptWithdrawal(context: context, id: trxID);
    Navigator.pop(context);
    if (_error.any) {
      Navigator.pop(context);
