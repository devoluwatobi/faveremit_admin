  ...
  void initState() {
      ...
      children: [
        Text(
          ...
          style: GoogleFonts.poppins(
              fontWeight: FontWeight.w600, fontSize: 15, color: kPrimaryColor),
        ),
        const SizedBox(
          height: 10,
        ),
        CachedNetworkImage(
          imageUrl: widget.giftCardModel.image.toString(),
          fit: BoxFit.contain,
        ),
      ],
    );
    _giftcardLogo = Column(
      children: [
        ...
        CachedNetworkImage(
          imageUrl: widget.giftCardModel.brandLogo.toString(),
          fit: BoxFit.contain,
        ),
      ],
    );
    _titleController.text = widget.giftCardModel.title;
                        ...
                        onTap: () async {
                          ...
                          if (result != null) {
                            ...
                            _giftcardLogo = Column(
                              children: [
                                Text(
                                  "Tap to change change Giftcard Logo",
                                  style: GoogleFonts.poppins(
                                      fontWeight: FontWeight.w600,
                                      fontSize: 15,
                                      color: kPrimaryColor),
                  ...
                  if (_formKey.currentState!.validate()) {
                    {
                      bool? _proceed = await showOptionPopup(
                          ...
                          body:
                              "Are you sure you want to update this giftcard?",
                          actionTitle: "I'm Sure",
                          isDestructive: true);

                      if (_proceed != null && _proceed) {
                        showLoadingModal(
                            context: context, title: "Updating Giftcard");
                        ProcessError _error = await adminWorker.updateGiftcard(
                            id: widget.giftCardModel.id,
                            title: _titleController.text.trim(),
                            image: _giftcardImageFile,
                            logo: _giftcardLogoFile,
                            hasImage: _giftcardImageFile != null,
                            hasLogo: _giftcardLogoFile != null,
                            context: context);
                        Navigator.pop(context);
                        if (_error.any) {
